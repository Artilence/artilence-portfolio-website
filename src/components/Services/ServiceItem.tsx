	...
	useEffect(() => {
		const handleScroll = () => {
			if (serviceItemRef.current) {
				...
				const serviceItemTop =
					serviceItemRef.current.getBoundingClientRect().top;
				const serviceItemHeight = serviceItemRef.current.clientHeight;

				if (
					serviceItemTop < viewportHeight &&
					serviceItemTop + serviceItemHeight > 0
				) {
					const visibleHeight = Math.min(
						serviceItemHeight,
						viewportHeight - serviceItemTop,
					);
		...
		<div ref={serviceItemRef}>
			<div className="max-w-[1676px] mx-auto flex flex-col xl:flex-row 2xl:min-h-screen gap-[305px] py-[10%] w-[85.78%] mx-auto ">
				...
				<div className={`flex-1 flex justify-center items-center`}>
					<div className={`flex-1 grid grid-cols-3 gap-x-[50px] gap-y-6`}>
